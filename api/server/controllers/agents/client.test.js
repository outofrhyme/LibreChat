...
describe('AgentClient.assembleSummarizedPayload', () => {
  test('preferred path requires full 2-user + 1-assistant match', () => {
    const messages = [
      new SystemMessage('AGENT-SYSTEM'),
      new HumanMessage('OLDER'),
      new HumanMessage('APPLE-ALPHA'),
      new AIMessage('BANANA-BETA'),
      new HumanMessage('CARROT-GAMMA'),
      new HumanMessage('CURRENT-PROMPT'),
    ];

    const protectedIndices = AgentClient.getProtectedTailIndices(messages, messages.length - 1);
    const protectedContents = Array.from(protectedIndices)
      .sort((a, b) => a - b)
      .map((i) => String(messages[i].content));

    expect(protectedContents).toEqual(['APPLE-ALPHA', 'BANANA-BETA', 'CARROT-GAMMA']);
  });

  test('falls back gracefully when only one prior non-system message exists', () => {
    const messages = [
      new SystemMessage('AGENT-SYSTEM'),
      new HumanMessage('APPLE-ALPHA'),
      new HumanMessage('CURRENT-PROMPT'),
    ...
    expect(protectedContents).toEqual(['APPLE-ALPHA']);
    expect(protectedIndices.size).toBe(1);
  });

  test('falls back when only one user + one assistant exist before current prompt', () => {
    const messages = [
      new SystemMessage('AGENT-SYSTEM'),
      new HumanMessage('APPLE-ALPHA'),
      new AIMessage('BANANA-BETA'),
    ...
    expect(protectedContents).toEqual(['APPLE-ALPHA', 'BANANA-BETA']);
    expect(protectedIndices.size).toBeGreaterThanOrEqual(2);
  });

  test('falls back when only assistant messages exist before current prompt', () => {
    const messages = [
      new SystemMessage('AGENT-SYSTEM'),
      new AIMessage('A1'),
      new AIMessage('A2'),
    ...
    const protectedIndices = AgentClient.getProtectedTailIndices(messages, messages.length - 1);
    expect(protectedIndices.size).toBe(3);
  });

  test('summarization active with enough room keeps summary context + protected sentinel tail + current prompt', () => {
    const messages = [
      new SystemMessage('AGENT-SYSTEM'),
      new HumanMessage('OLDER-ZETA'),
      new AIMessage('OLDER-ETA'),
      new HumanMessage('APPLE-ALPHA'),
      new AIMessage('BANANA-BETA'),
      new HumanMessage('CARROT-GAMMA'),
      new HumanMessage('CURRENT-PROMPT'),
    ...
    const result = AgentClient.assembleSummarizedPayload(messages, {
      text: 'Conversation summary',
    });

    const finalText = result.messages.map((m) => String(m.content)).join('\n');
    expect(finalText).toContain('AGENT-SYSTEM');
    expect(finalText).toContain('APPLE-ALPHA');
    expect(finalText).toContain('BANANA-BETA');
    expect(finalText).toContain('CARROT-GAMMA');
    expect(finalText).toContain('CURRENT-PROMPT');
    expect(finalText).not.toContain('OLDER-ZETA');
    expect(finalText).not.toContain('OLDER-ETA');
  });

  test('summarization active + tight room fallback path preserves 3 recent non-system messages when role mix is absent', () => {
    const messages = [
      new SystemMessage('AGENT-SYSTEM'),
      new HumanMessage('OLDER-A'),
      new HumanMessage('OLDER-B'),
      new HumanMessage('APPLE-ALPHA'),
      new HumanMessage('BANANA-BETA'),
      new HumanMessage('CARROT-GAMMA'),
      new HumanMessage('CURRENT-PROMPT'),
    ];
    ...
    expect(finalText).toContain('CARROT-GAMMA');
    expect(finalText).toContain('CURRENT-PROMPT');
    expect(finalText).not.toContain('OLDER-A');
    expect(finalText).not.toContain('OLDER-B');
  });

  test('no summarization path remains unchanged', () => {
    const messages = [
    ...
    const result = AgentClient.assembleSummarizedPayload(messages, null);
    expect(result.messages).toEqual(messages);
    expect(result.currentPromptIndex).toBe(-1);
    expect(result.protectedIndices.size).toBe(0);
  });
});

describe('AgentClient.chatCompletion summarized payload path', () => {
  let client;
  ...
  beforeEach(() => {
    ...
    createRun.mockResolvedValue({
      ...
      getRunMessages: jest.fn().mockReturnValue([]),
    });
    ...
    client.loadSpecs = jest.fn().mockResolvedValue(undefined);
    client.parentMessageId = 'msg-current';
    client.conversationId = 'convo-1';
    client.responseMessageId = 'resp-1';
    client.user = 'user-1';
  });

  test('final createRun payload preserves APPLE/BANANA/CARROT sentinels with summary present', async () => {
    const agentsModule = require('@librechat/agents');
    const formatSpy = jest.spyOn(agentsModule, 'formatAgentMessages').mockReturnValue({
      messages: [
        new SystemMessage('SYSTEM-CONTEXT'),
        new HumanMessage('OLDER-DROP-ME'),
        new HumanMessage('APPLE-ALPHA'),
        new AIMessage('BANANA-BETA'),
        new HumanMessage('CARROT-GAMMA'),
        new HumanMessage('CURRENT-PROMPT'),
      ],
      indexTokenCountMap: { 0: 5, 1: 5, 2: 5, 3: 5, 4: 5, 5: 5 },
      summary: { text: 'summary text', tokenCount: 10 },
    });

    await client.chatCompletion({ payload: [] });

    const callArgs = createRun.mock.calls[0][0];
    const finalPayload = callArgs.messages.map((m) => String(m.content)).join('\n');
    expect(finalPayload).toContain('APPLE-ALPHA');
    expect(finalPayload).toContain('BANANA-BETA');
    expect(finalPayload).toContain('CARROT-GAMMA');
    expect(finalPayload).not.toContain('OLDER-DROP-ME');
