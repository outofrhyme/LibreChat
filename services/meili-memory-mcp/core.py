...
USER_ID_HEADER_ALIASES = (USER_ID_HEADER, "x-user-id", "user-id")
    ...
    def normalize_asgi_headers(headers: Sequence[Any]) -> dict[str, str]:
        ...
        for item in headers:
            if not isinstance(item, Sequence) or len(item) != 2:
                continue
            key, value = item
            normalized[decode_header_part(key).lower()] = decode_header_part(value)
        return normalized
...
def build_sender_filter(sender_values: list[str]) -> str | None:
    ...
    return "(" + " OR ".join(f"sender = '{sender}'" for sender in escaped) + ")"
...
def collect_sender_variants_for_agent(
    ...
    if not facets or not normalized_agent_name:
        return []
